...
This document catalogs all user-facing strings in the UnifiWatch application that require localization. Use this guide when adding support for new languages or maintaining existing translations.

**For Contributors**: This catalog helps you:
- Identify all strings that need translation for a new language
- Understand the context and usage of each string
- Ensure complete localization coverage (no missing translations)
- Map strings to the correct resource files (CLI.*.json, Notifications.*.json, Errors.*.json)

**Existing Translations**: Currently supports 6 languages:
- English (default)
...
See [LOCALIZATION_GUIDELINES.md](LOCALIZATION_GUIDELINES.md) for translation standards and best practices.
...
1. **Choose your language code**: Use ISO 639-1 (language) + ISO 3166-1 (country) format
   - Examples: `pt-BR` (Portuguese-Brazil), `ja-JP` (Japanese-Japan), `zh-CN` (Chinese-China)

2. **Create resource files** in `Resources/` directory:
   - `CLI.{language-code}.json` - Command-line help text
   - `Notifications.{language-code}.json` - Notification messages
   - `Errors.{language-code}.json` - Error messages

3. **Translate all strings** listed in this catalog:
   - Use sections below to find all strings requiring translation
   - Preserve placeholder syntax: `{name}`, `{sku}`, `{error}`, etc.
   - Maintain consistent terminology across all resource files
   - See LOCALIZATION_GUIDELINES.md for quality standards

4. **Test your translations**:
   - Set system language to your new language
   - Run application and verify all strings display correctly
...
**For Maintainers**: When adding new user-facing strings to the codebase:

1. Add the string to all existing language resource files
2. Update this catalog with the new string, its location, and context
3. Update the "Last Updated" date at the top of this file
4. Consider opening a GitHub issue requesting community translations

**Created**: December 12, 2025 (Original Phase 2 Audit)  

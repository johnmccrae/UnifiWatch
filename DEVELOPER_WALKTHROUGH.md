# UnifiWatch - Developer Walkthrough

A comprehensive guide to understanding the UnifiWatch codebase. This document walks through the architecture, design patterns, and implementation details to help developers understand how all the moving parts work together.

**Intended Audience**: Developers with C# experience who want to understand the UnifiWatch architecture and contribute to the project.
...
├── Models/                             ← Data models
├── Services/
│   ├── NotificationService.cs          ← Main notification interface
│   ├── StockWatcher.cs                 ← Background service
│   ├── UnifiStockService.cs            ← API client (primary)
│   ├── UnifiStockLegacyService.cs      ← API client (fallback)
...
**What happens**:
1. C# compiler compiles all `.cs` files
2. NuGet restores dependencies
3. Output goes to `bin/Debug` (default)

**Common issues**:
- Missing .NET 9 SDK → Install from dotnet.microsoft.com
- NuGet package conflicts → Delete `bin/` and `obj/`, rebuild
...
**What happens**:
1. xUnit test framework discovers all `*Tests.cs` files
2. Runs all test methods (those with `[Fact]` or `[Theory]` attributes)
...
UnifiWatch has comprehensive test coverage (146 tests).
...
dotnet test UnifiWatch.Tests/EmailNotificationServiceTests.cs

# Run specific test
dotnet test --filter "TestClassName=EmailNotificationServiceTests"

# Run with coverage
dotnet test /p:CollectCoverage=true

# Run verbose (show each test)
